...
import type { Database, Runtime, Serializer, TelegramClient } from "../ports.ts";
...
export class UpdateProcessor {
  ...
  /** Entry point: dedupe, then serialize by conversation, then handle. */
  async process(updateId: number, event: InboundEvent): Promise<ProcessResult> {
    if (event.kind === "ignored") {
      this.logger.info("update_ignored", { updateId });
      return { status: "ignored" };
    }

    const claimed = await this.db.processedUpdates.claim(updateId, this.runtime.now());
    if (!claimed) {
      this.logger.info("update_duplicate", { updateId });
    ...
    this.logger.info("update_received", { updateId });
    ...
    try {
      result = await this.serializer.runExclusive(this.keyFor(event), () => this.processEvent(updateId, event));
    } catch (err) {
  ...
  private async processEvent(updateId: number, event: InboundEvent): Promise<ProcessResult> {
    switch (event.kind) {
      case "user_message":
        return this.handleUserMessage(updateId, event);
      case "operator_message":
        return this.handleOperatorMessage(event);
      case "edited_user_message":
        return this.handleEditedUserMessage(event);
      case "edited_operator_message":
        return this.handleEditedOperatorMessage(event);
      case "verification_answer":
        return this.handleVerificationAnswer(event);
      case "application_decision":
        return this.handleApplicationDecision(event);
      case "conversation_delete":
        return this.handleConversationDelete(event);
      default:
        return { status: "ignored" };
    }
  }
  ...
  private async handleUserMessage(updateId: number, event: UserMessageEvent): Promise<ProcessResult> {
    ...
    // passed human verification are subject to the blacklist — a verified human
    // is trusted and may say e.g. "加微信" without being auto-blocked. A hit
    // drops the message, auto-blocks the user (per config) and notifies the
    // support group.
    ...
    // or approval. Shared by the ad-text check (2b) and the verification gate (5).
    const gated = existing == null || this.deps.verification.isEligible(existing);
    if (gated) {
      const adCheck = await this.deps.ad.detect(event.content);
      if (adCheck.detected) {
        await this.enforceAd(event, adCheck);
    ...
    if (event.content.type === "text" && event.content.text.startsWith("/")) {
      return this.handleUserCommand(event);
    }
    ...
    if (!isStaff && gated) {
      ...
      await this.deps.pending.enqueue(event.sender.telegramUserId, event.messageId, event.content.type, event.replyToMessageId);
      await this.deps.verification.nonButtonContent(event.sender);
    ...
    if (isStaff && this.deps.verification.isEligible(user)) {
      // Staff never see the challenge; record it so a later registry removal
      // (no longer staff) doesn't re-gate a known trusted human.
      await this.deps.users.markVerified(user.telegramUserId);
    }
    ...
    if (await this.purposeGatePending(user, isStaff)) {
      const conversation = await this.recordPurposeAndOpen(user, event);
      await this.flushPending(user, conversation);
  ...
  private async handleUserCommand(event: UserMessageEvent): Promise<ProcessResult> {
    ...
    if (cmd === "/start") {
      ...
      if (!isStaff && (!existing || this.deps.verification.isEligible(existing))) {
        ...
        await this.deps.verification.nonButtonContent(sender);
        return { status: "verification_issued" };
      }
    ...
    switch (cmd) {
      case "/apply": {
        const outcome = await this.deps.approvals.apply(sender);
      ...
      case "/help": {
        ...
        await this.telegram.sendMessage({ chatId: sender.telegramUserId, text: operator ? staff.helpGeneral : user.userHelp });
        break;
      }
      case "/lang": {
        ...
        await this.deps.commands.lang(sender, args, { chatId: sender.telegramUserId });
        break;
      }
      case "/selfcheck": {
        ...
        const { operator, user, staff } = await this.commandTextsOf(sender);
        if (!operator) {
          await this.telegram.sendMessage({ chatId: sender.telegramUserId, text: user.userHelp });
          break;
        }
        if (!(await this.deps.operators.isAdmin(sender.telegramUserId))) {
          await this.telegram.sendMessage({ chatId: sender.telegramUserId, text: staff.adminOnly });
          break;
        }
        const report = await this.deps.selfCheck.run();
        await this.telegram.sendMessage({ chatId: sender.telegramUserId, text: staff.selfcheckReport(report) });
      ...
      case "/delete": {
        ...
        if (cmd === "/list") {
          await this.deps.commands.listAll(sender, event.chatId);
        } else {
          await this.deps.commands.deleteFromList(sender, event.chatId, args);
        }
        break;
      }
      default: {
    ...
    await this.repeatPurposePromptIfPending(sender);
    return { status: "command_handled" };
  }

  private async handleOperatorMessage(event: OperatorMessageEvent): Promise<ProcessResult> {
  ...
  private async handleEditedUserMessage(event: EditedUserMessageEvent): Promise<ProcessResult> {
    ...
    const existing = await this.deps.users.getByTelegramUserId(event.sender.telegramUserId);
    if (existing == null || this.deps.verification.isEligible(existing)) {
      const adCheck = await this.deps.ad.detect(event.content);
      if (adCheck.detected) {
        await this.enforceAd(event, adCheck);
        return { status: "message_rejected" };
  ...
  private async handleEditedOperatorMessage(event: EditedOperatorMessageEvent): Promise<ProcessResult> {
    // Same surface rule as operator messages: the support group only, and only
    // registered operators may edit-deliver into a user's chat.
    if (event.chatId !== this.config.supportGroupId) return { status: "ignored" };
    if (!(await this.deps.operators.isOperator(event.sender.telegramUserId))) {
  ...
  private async handleVerificationAnswer(event: VerificationAnswerEvent): Promise<ProcessResult> {
    const t = TEXTS(await effectiveLanguageOf(this.db, event.sender));
    if (await this.db.blocks.getByTelegramUserId(event.sender.telegramUserId)) {
      await this.answerCallback(event.callbackQueryId, t.verifyBlocked, true);
      return { status: "blocked" };
    }
    if (!(await this.spamAllowed(event.sender.telegramUserId))) {
      await this.answerCallback(event.callbackQueryId, t.tooManyRequests, true);
      return { status: "rate_limited" };
    }

    const outcome = await this.deps.verification.answer(event.sender, event.answer);
    switch (outcome.outcome) {
      case "correct": {
        ...
        await this.telegram
          ...
          .catch(() => {});
        await this.answerCallback(event.callbackQueryId, t.verifyCorrect, false);
        ...
        if (user.purpose == null) {
          await this.telegram.sendMessage({ chatId: event.chatId, text: t.purposePrompt });
          this.logger.info("purpose_prompted", { telegramUserId: user.telegramUserId });
          return { status: "purpose_pending" };
        }
        const conversation = await this.deps.conversations.grantAccess(user);
      ...
      case "wrong":
        await this.answerCallback(event.callbackQueryId, t.verifyWrong, false);
        return { status: "verification_issued" };
      case "exhausted":
        await this.telegram.sendMessage({ chatId: event.chatId, text: t.verifyExpired });
        await this.answerCallback(event.callbackQueryId, t.outOfAttempts, true);
        return { status: "message_rejected" };
      case "expired":
        await this.telegram.sendMessage({ chatId: event.chatId, text: t.verifyExpired });
        await this.answerCallback(event.callbackQueryId, t.challengeExpired, true);
        return { status: "message_rejected" };
      case "no_challenge":
        await this.answerCallback(event.callbackQueryId, t.noActiveChallenge, true);
        return { status: "ignored" };
    }
  }

  private async handleApplicationDecision(event: ApplicationDecisionEvent): Promise<ProcessResult> {
  ...
  private async handleConversationDelete(event: ConversationDeleteEvent): Promise<ProcessResult> {
    await this.deps.commands.handleDeleteTap(event);
    return { status: "command_handled" };
  }
  ...
  private async recordPurposeAndOpen(user: UserRecord, event: UserMessageEvent): Promise<ConversationRecord> {
    const fresh = await this.deps.users.setPurpose(user.telegramUserId, purposeTextOf(event.content));

    const conversation = await this.deps.conversations.grantAccess(fresh, { postCard: false });
  ...
  private async repeatPurposePromptIfPending(sender: UserProfile): Promise<void> {
  ...
  private keyFor(event: InboundEvent): string {
    switch (event.kind) {
      case "user_message":
        return `u:${event.sender.telegramUserId}`;
      case "edited_user_message":
        return `u:${event.sender.telegramUserId}`;
      case "verification_answer":
        return `u:${event.sender.telegramUserId}`;
      case "operator_message":
        return `t:${event.messageThreadId ?? "general"}`;
      case "edited_operator_message":
        return `t:${event.messageThreadId ?? "general"}`;
      case "application_decision":
        return `app:${event.applicationId}`;
      case "conversation_delete":
        return `del:${event.conversationId}`;
      case "ignored":
        return "ignored";
    }
  }
   ...
   *  chatId + messageId). */
  private async enforceAd(event: { sender: UserProfile; content: MessageContent; chatId: number; messageId: number }, check: AdCheckResult): Promise<void> {
    ...
    // admin is never left blind by an ad hit.
   ...
   *  their conversation is open. See PendingService for what gets queued. */
  private async flushPending(user: UserRecord, conversation: ConversationRecord): Promise<void> {
    const entries = await this.deps.pending.drain(user.telegramUserId);
