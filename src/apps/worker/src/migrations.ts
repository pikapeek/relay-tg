...
//
// Workers cannot `fs`-read `migrations/*.sql` and Wrangler's bundler rejects
// Vite-style `?raw` imports, so the schema is embedded here as a plain module.
// The drift test in index.test.ts reads `migrations/001_initial.sql` from disk
// and asserts it equals this string, so the two cannot silently diverge.
// ---------------------------------------------------------------------------

import type { Migration } from "@relaytg/adapter-sqlite/migrate";
